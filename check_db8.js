...
async function check() {
  const { data } = await supabase.from('registro_trabajo').select('id,tipo_trabajo,created_date,fase,es_finalizado').order('id', { ascending: false }).limit(10);
  console.log("Newest inserts by ID:", data);
}
check();
