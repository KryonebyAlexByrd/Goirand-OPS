...
async function check() {
  const { data, error } = await supabase.from('registro_trabajo').select('id,proyecto_id,proyecto_nombre,tipo_trabajo,es_finalizado').order('id', { ascending: false }).limit(5);
