...
async function insertProject() {
  const newProject = {
    id: generateSafeId(),
    numero_proyecto: "ENE26-09-02",
    descripcion: "Ritz-Carlton Reserve Costa Canuva - Branded",
    cliente_nombre: "Katia Cesar",
    estado: "Activo",
    prioridad: "Alta",
    fecha_inicio: "2026-01-09",
    monto_total: 18084922,
    monto_incluye_iva: true,
    fase_actual: "1. Recepción de Proyecto",
    porcentaje_avance: 0,
    partidas_cotizacion: partidas,
    fabricas_asignadas: ["Tecomatla"],
    contratistas_asignados: [],
    pagos_programados: [],
    fases_completadas: [],
    notas_internas: "Proyecto registrado automáticamente vía Script con 46 partidas extraídas de la cotización original."
  };

  const { data, error } = await supabase.from('proyecto').insert(newProject).select();
  if (error) {
    console.error("ERROR INSERTANDO PROYECTO:", error);
  } else {
    console.log("¡PROYECTO INSERTADO EXITOSAMENTE!", data[0].id);
  }
}
